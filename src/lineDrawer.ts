...
import { acos, cross, EPSILON, INFINITY } from "three/tsl";
...
export class Bilbord {
  ...
  constructor(Ain: THREE.Vector3, Bin: THREE.Vector3) {
    this.A = Ain;
    this.B = Bin;
  }

  generateMesh(camPos: THREE.Vector3): THREE.Vector3[] {
    const Mesh: THREE.Vector3[] = [];

    let AB = new THREE.Vector3().subVectors(this.A, this.B);
    if (this.A.equals(this.B) || AB.length() < 1e-6) {
      console.warn(
        "Tried to make a bilboard between same or really close points",
      );
      return Mesh;
    }
...
export class lineDrawer {
  ...
  public angleBuffer: angle[] = [];
  public undoBuffer: number[] = [];
  ...
  inter1: THREE.Vector3 | null = null;
  inter2: THREE.Vector3 | null = null;
  inter3: THREE.Vector3 | null = null;

  public drawingType: drawTypes = drawTypes.lines;
  ...
  public addMarker(pos: THREE.Vector3) {
  ...
  public bilboardRaycast(
    mX: number,
    mY: number,
    points: THREE.Vector3[],
    camera: OrbitCamera,
    quickRaycastSnap: boolean,
  ) {
    var bilbords = this.bilbordgen(points, camera, 1);
    var linePoints: THREE.Vector3[] = [];
    ...
    this.raycaster.set(
      camera.getPosition(),
      camera.screenToWorldRay(mX, mY, window.innerWidth, window.innerHeight),
    );

    const intersects = this.raycaster.intersectObjects(bilbords.children, true);

    if (intersects.length > 0) {
      var snapTable = this.generateSnapTable(points).concat(
        this.generateLineSnapTable(linePoints),
      );
      const pointsForSnapping: THREE.Vector3[] = snapTable;
      ...
      if (this.inter1 == null) {
        ...
        this.inter1 = this.findClosestPoint(
          this.inter1!,
          pointsForSnapping,
          intersects[0].object,
        );
        ...
        this.inter2 = this.findClosestPoint(
          this.inter2!,
          pointsForSnapping,
          intersects[0].object,
        );
        ...
        if (
          this.inter1.distanceTo(this.inter2) > 1e-6 &&
          this.drawingType === drawTypes.lines
        ) {
          this.lineBuffer.push({
            A: this.inter1,
            B: this.inter2,
            color: this.scene.lineMaterial.color.clone(),
          } as line);
        ...
        this.inter3 = this.findClosestPoint(
          this.inter3!,
          pointsForSnapping,
          bilbords,
        );
        this.angleBuffer.push({
          ...
          B: this.inter2,
          C: this.inter3,
          color: this.scene.angleMaterial.color.clone(),
        } as angle);
  ...
  getLineIntersection(
    ...
    const d1 = p2.clone().sub(p1); // direction of segment 1
    const d2 = p4.clone().sub(p3); // direction of segment 2
    ...
    // If r x s == 0 and (q - p) x r == 0 → segments are collinear
    if (rxsLenSq < epsilon) {
      if (qmpCrossR.lengthSq() < epsilon) {
        ...
        if (Math.max(0, Math.min(t0, t1)) <= Math.min(1, Math.max(t0, t1))) {
          // collinear and overlapping — return the midpoint of the common interval
          const t =
            (Math.max(0, Math.min(t0, t1)) + Math.min(1, Math.max(t0, t1))) / 2;
          return p1.clone().add(r.multiplyScalar(t));
        }
      }
      return null; // parallel or collinear but not intersecting
    }

    // If segments are not parallel — compute the intersection point of the lines
    ...
    // If t and u are in [0,1], the segments intersect
    if (
      t >= -epsilon &&
      t <= 1 + epsilon &&
      u >= -epsilon &&
  ...
  bilbordgen(
    ...
    for (let i = 0; i < points.length - 1; i += skips) {
      ...
      const camPos: THREE.Vector3 = camera.getPosition();
  ...
  genSnapToLine(bil: THREE.Object3D, point: THREE.Vector3): THREE.Vector3 {
    const mesh = bil as THREE.Mesh;
    const userData = mesh.userData as { A: THREE.Vector3; B: THREE.Vector3 };
  ...
  generateSnapTable(points: THREE.Vector3[]) {
    let snapTable: THREE.Vector3[] = [];

    // All vertexes + middles
    var i = 0;
    while (i < points.length - 1) {
      var theMiddle = new THREE.Vector3().lerpVectors(
        points[i],
        points[i + 1],
        0.5,
      );
    ...
    for (let i = 0; i < this.lineBuffer.length - 1; i++) {
      for (let j = i + 1; j < this.lineBuffer.length; j++) {
        ...
        if (intersection) snapTable.push(intersection);
      }
    }
  ...
  generateLineSnapTable(points: THREE.Vector3[]) {
    let snapTable: THREE.Vector3[] = [];
  ...
  generateAngle(
    ...
    if (isRightAngle) {
      ...
      const corner2 = B.clone()
        .add(right.clone().multiplyScalar(radius))
        .add(up.clone().multiplyScalar(radius));
      ...
      for (let i = 0; i <= 32; i++) {
        ...
        let point = B.clone()
          .add(right.clone().multiplyScalar(radius * Math.cos(angle)))
          .add(up.clone().multiplyScalar(radius * Math.sin(angle)));
        vertices.push(point);
      }
    }
