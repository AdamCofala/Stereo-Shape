...
export class Shape {
    constructor() {
        this.geometrySingle = [];
        this.circleMid = new THREE.Vector3(0, 0, 0);
        this.radius = 0;
    }
    isRightAngle(A, B, // Angle at point B
    C, epsilon = 1e-9) {
    ...
    addRightAngleMarker(vertices, A, B, // This is now the ANGLE VERTEX
    C, markerSize = 0.2) {
        if (!this.isRightAngle(A, B, C))
            return;
    ...
    getRightAngleMarker(A, B, C) {
        const rightAngleVertices = [];
    ...
    createWalls(vertices, A, B, C, h, in_circle, out_circle) {
    ...
    createTrianglesVertices(a, b, c, h, type, nangle, in_circle, out_circle) {
        const vertices = [];
        this.radius = 0;
        switch (type) {
            case 0: //any triangle
