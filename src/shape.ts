...
export class Shape {
  ...
  private addRightAngleMarker(
    ...
    // Get vectors FROM B (clone to avoid modifying originals)
    const BA = new THREE.Vector3().subVectors(A, B).normalize();
    const BC = new THREE.Vector3().subVectors(C, B).normalize();

    // Create the right angle marker square
    const corner1 = new THREE.Vector3()
      .copy(B)
      .add(BA.clone().multiplyScalar(markerSize));
    const corner2 = new THREE.Vector3()
      .copy(corner1)
      .add(BC.clone().multiplyScalar(markerSize));
    const corner3 = new THREE.Vector3()
      .copy(B)
      .add(BC.clone().multiplyScalar(markerSize));

    // Add lines forming the marker square
  ...
  public getRightAngleMarker(
    ...
    // Check all possible right angles in the triangle
    this.addRightAngleMarker(rightAngleVertices, A, B, C); // Angle at vertex B
    this.addRightAngleMarker(rightAngleVertices, B, A, C); // Angle at vertex A
    this.addRightAngleMarker(rightAngleVertices, C, A, B); // Angle at vertex A (alt)
    this.addRightAngleMarker(rightAngleVertices, A, C, B); // Angle at vertex C
    this.addRightAngleMarker(rightAngleVertices, B, C, A); // Angle at vertex C (alt)
    this.addRightAngleMarker(rightAngleVertices, C, B, A); // Angle at vertex B (alt)
