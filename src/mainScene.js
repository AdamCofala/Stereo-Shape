...
export default class mainScene extends THREE.Scene {
    constructor(renderer) {
        ...
        this.activeLine = new Line2();
        this.activeLineVisibility = false;
        // Public variables
        this.vertecies = [];
        this.hideCursor = 1;
        this.charArray = [];
        ...
        this.circleMaterial = new LineMaterial({
            color: 0x9500f4,
            linewidth: 3,
            resolution: new THREE.Vector2(window.innerWidth, window.innerHeight),
            transparent: true,
        ...
        this.angleMaterial = new LineMaterial({
            color: 0xffdf,
            linewidth: 3,
            resolution: new THREE.Vector2(window.innerWidth, window.innerHeight),
            transparent: true,
        ...
        this.mainCamera = new OrbitCamera({ position: new THREE.Vector3(0, 5, 10) });
    ...
    createShape() {
        this.clear();
        this.addLight();
        // Generuj nowy kształt
        const points = this.shape.createTrianglesVertices(this.params.a, this.params.b, this.params.c, this.params.h, this.params.type, this.params.nangle, this.params.in_circle, this.params.out_circle);
        ...
        const rightAnglePoints = this.shape.getRightAngleMarker(points[0], points[1], points[2]);
        if (rightAnglePoints.length > 0) {
            const rightAngle = this.createLine2(rightAnglePoints, this.material, false);
            this.add(rightAngle);
        }
        this.drawLines();
        this.drawAngles();
        if (this.activeLineVisibility)
            this.drawActiveLine();
    }
    drawLines() {
        let i = 0;
        while (i < this.linedrawer.lineBuffer.length) {
            const points = [this.linedrawer.lineBuffer[i].A, this.linedrawer.lineBuffer[i].B];
    ...
    drawAngles() {
        ...
        while (i < this.linedrawer.angleBuffer.length) {
            const points = this.linedrawer.generateAngle(this.linedrawer.angleBuffer[i].A, this.linedrawer.angleBuffer[i].B, this.linedrawer.angleBuffer[i].C);
            const newlineMaterial = new LineMaterial({
    ...
    drawActiveLine() {
        ...
        const activeLineGeomatry = new LineGeometry().setFromPoints(this.activeLinePoinst);
    ...
    activeLineUpdate(A) {
        if (this.activeLinePoinst.length === 2) {
            ...
            const positions = [
                this.activeLinePoinst[0].x, this.activeLinePoinst[0].y, this.activeLinePoinst[0].z,
                this.activeLinePoinst[1].x, this.activeLinePoinst[1].y, this.activeLinePoinst[1].z
            ];
    ...
    createLine2(points, material, closed = false) {
        const positions = [];
    ...
    captureScreenshotWithCanvasCrop(padding = 20) {
        ...
        gl.readPixels(0, 0, tempWidth, tempHeight, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
        // Odwróć obraz pionowo
        ...
        for (let y = 0; y < tempHeight; y++) {
            ...
            flippedPixels.set(pixels.subarray(srcStart, srcStart + tempWidth * 4), destStart);
        }
        // Znajdź granice nieprzezroczystych pikseli
        ...
        // Stwórz pomocniczy canvas do konwersji Uint8Array → ImageData
        const tempCanvas = document.createElement('canvas');
        tempCanvas.width = tempWidth;
        tempCanvas.height = tempHeight;
        const tempCtx = tempCanvas.getContext('2d');
        const imageData = new ImageData(new Uint8ClampedArray(flippedPixels), tempWidth, tempHeight);
        tempCtx.putImageData(imageData, 0, 0);
        // Przekopiuj wycinek z tymczasowego canvasa
    ...
    findContentBounds(pixels, width, height) {
