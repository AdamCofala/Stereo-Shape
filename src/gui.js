...
export class handleGUI {
    constructor(sceneToControl) {
        this.gui = null; // Zmienione na private i nullable
        this.selectedButton = null;
        this.linesButton = null;
        this.anglesButton = null;
    ...
    setupGUI() {
        ...
        this.gui = new GUI({
            autoPlace: true,
            hideable: false, // Wyłącz możliwość ukrywania
            closed: false, // Zawsze otwarte
            closeOnTop: false // Usuń przycisk zamykania z góry
        });
        const shapeFolder = this.gui.addFolder('Ustawienia figury');
        shapeFolder.open();
        const state = { act: 'Dowolny trójkąt' };
        const typeOptions = [
            'Dowolny trójkąt',
            'Trójkąt prostokątny',
        ...
        shapeFolder.add(state, 'act').options(typeOptions).name('Typ podstawy / figury').onChange(() => {
            this.scene.params.type = typeOptions.indexOf(state.act);
            this.scene.clearAll();
            this.scene.createShape();
        });
        // Parametry wymiarów
        const paramsFolder = shapeFolder.addFolder('Wymiary');
        paramsFolder.add(this.scene.params, 'a', 0.1, 6).step(0.1).name('Bok A').onChange(() => { this.scene.clearAll(), this.scene.createShape(); });
        paramsFolder.add(this.scene.params, 'b', 0.1, 6).step(0.1).name('Bok B').onChange(() => { this.scene.clearAll(), this.scene.createShape(); });
        paramsFolder.add(this.scene.params, 'c', 0.1, 6).step(0.1).name('Bok C').onChange(() => { this.scene.clearAll(), this.scene.createShape(); });
        paramsFolder.add(this.scene.params, 'h', 0.1, 6).step(0.1).name('Wysokość').onChange(() => { this.scene.clearAll(), this.scene.createShape(); });
        paramsFolder.add(this.scene.params, 'nangle', 3, 16).step(1).name('Liczba boków').onChange(() => { this.scene.clearAll(), this.scene.createShape(); });
        paramsFolder.open();
        // Okręgi
        const circleFolder = shapeFolder.addFolder('Okręgi na postawie');
        circleFolder.add(this.scene.params, 'in_circle')
            .name('Okrąg wpisany')
            .listen()
            .onChange((value) => {
            if (value) {
        ...
        circleFolder.add(this.scene.params, 'out_circle')
            .name('Okrąg opisany')
            .listen()
            .onChange((value) => {
            if (value) {
        ...
        circleFolder.open();
        const visualsFolder = this.gui.addFolder('Wizualia i obraz');
        const lineFolder = visualsFolder.addFolder('Grubość linii');
        const lineParams = {
            mainLineWidth: 3,
            circleLineWidth: 3
        };
        lineFolder.add(lineParams, 'mainLineWidth', 1, 20).step(1)
            .name('Główne linie')
            .onChange((value) => {
            this.scene.material.linewidth = value;
            this.scene.createShape();
        });
        lineFolder.add(lineParams, 'circleLineWidth', 1, 20).step(1)
            .name('Okręgi')
            .onChange((value) => {
        ...
        lineFolder.open();
        const colorFolder = visualsFolder.addFolder('Kolory');
        const colorParams = {
            Figura: this.scene.material.color.getHex(),
            Okrąg: this.scene.circleMaterial.color.getHex(),
            Linia: this.scene.lineMaterial.color.getHex(),
            Kąty: this.scene.angleMaterial.color.getHex()
        };
        colorFolder.addColor(colorParams, 'Figura').onChange((value) => {
            this.scene.material.color.set(value);
            this.scene.createShape();
        });
        colorFolder.addColor(colorParams, 'Okrąg').onChange((value) => {
            this.scene.circleMaterial.color.set(value);
            this.scene.createShape();
        });
        colorFolder.addColor(colorParams, 'Linia').onChange((value) => {
            this.scene.lineMaterial.color.set(value);
            this.scene.drawLines();
        });
        colorFolder.addColor(colorParams, 'Kąty').onChange((value) => {
            this.scene.angleMaterial.color.set(value);
            this.scene.drawAngles();
        });
        colorFolder.open();
        let functions = { screenShot: () => this.scene.captureScreenshotWithCanvasCrop() };
        const imageFolder = visualsFolder.addFolder('Obraz');
        imageFolder.add(functions, 'screenShot').name('Zapisz obraz jako .png');
        imageFolder.open();
        visualsFolder.open();
    ...
    createCustomLeftPanel() {
        ...
        undo.style.cssText = buttonBaseStyle + `
        background: #2a2a3a;
    ...
    `;
        ...
        clearBtn.style.cssText = buttonBaseStyle + `
        background: #2a2a3a;
        font-family: 'Overpass';
        font-weight: bold;
        font-size: 12px;
